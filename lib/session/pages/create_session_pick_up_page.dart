...
class CreateSessionPickUpPage extends StatelessWidget {
  const CreateSessionPickUpPage({super.key});
  ...
  Widget build(BuildContext context) {
    return BlocProvider.value(
      ...
      child: const CreateSessionPickUpView(),
    );
  }
}

class CreateSessionPickUpView extends StatelessWidget {
  const CreateSessionPickUpView({super.key});
              ...
              return ListView(
                children: [
                  ListTile(
                    ...
                    subtitle: const Text(
                        'Informe o valor em dinheiro a ser retirado do caixa da sessão atual e clice no botão de confirmação.'),
                  ),
                  ListTile(
                    title: CurrencyField(
                      decoration: InputDecoration(
                        label: const Text('Valor a ser retirado do caixa'),
        ...
        builder: (context, state) {
          if (state.status != SessionStatus.closed) {
            return FloatingActionButton.extended(
              onPressed: () => context
                  .read<SessionBloc>()
                  .add(SessionPickedUp(amount: amount)),
